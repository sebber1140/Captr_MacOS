...
class OBSClient:
    ...
    def __init__(
        ...
        self.original_profile = None # Initialize
        target_profile = "computer_tracker"
        try:
            profile_list = self.req_client.get_profile_list()
            self.original_profile = profile_list.current_profile_name

            if target_profile not in profile_list.profiles:
                logging.info(f"Creating OBS profile: {target_profile}")
                self.req_client.create_profile(target_profile)
                # Refresh profile list after creation
                profile_list = self.req_client.get_profile_list()

            # Always try to switch to the target profile
            logging.info(f"Ensuring OBS profile is set to: {target_profile}")
            self.req_client.set_current_profile(target_profile)

            # Verify the switch happened (optional but good practice)
            time.sleep(0.1) # Give OBS a moment
            current_profile_check = self.req_client.get_profile_list().current_profile_name
            if current_profile_check != target_profile:
                logging.warning(f"Failed to verify switch to OBS profile {target_profile}. Current: {current_profile_check}")
                # Decide if this is critical? Maybe raise error?

        except Exception as e:
            logging.error(f"Error setting up OBS profile: {e}")
            # Keep original_profile as potentially None if setup failed early
            # Let initialization continue, settings might fail later
        # --- End Robust Profile Handling ---

        # Apply settings AFTER ensuring the correct profile is active
    ...
    def stop_recording(self):
        try:
            ...
            logging.error(f"Error sending stop_record command: {e}")
        # Remove profile switch from here - handled by restore_profile in cleanup
        # self.req_client.set_current_profile(self.original_profile)
    ...
    def restore_profile(self):
        # Check if original_profile was successfully captured
        if hasattr(self, 'original_profile') and self.original_profile:
            try:
                # Check current profile before attempting switch
                current_profile = self.req_client.get_profile_list().current_profile_name
                if current_profile != self.original_profile:
                    logging.info(f"Restoring original OBS profile: {self.original_profile}")
                    self.req_client.set_current_profile(self.original_profile)
                else:
                    logging.info("OBS profile already set to original.")
            except Exception as e:
                logging.error(f"Failed to restore original OBS profile: {e}")
        else:
            logging.warning("Original OBS profile name not found, cannot restore.")
